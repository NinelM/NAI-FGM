...
# nohup python -u my_si_ti_fgsm.py > my_si_ti_fgsm_inceptionv3.log 2>&1 &
...
def graph(x, y, i, x_max, x_min, grad, mt, vt):
    ...
    alpha_t = alpha_norm2 * num_iter * (tf.sqrt(1-tf.pow(beta2, tf.cast(i, dtype=tf.float32)+1)) 
        / (1- tf.pow(beta1, tf.cast(i, dtype=tf.float32)+1))) / weight
    ...
    pred = tf.argmax(end_points_v3['Predictions'] + end_points_v4['Predictions'] + end_points_res_v2['Predictions']
    ...
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v3_2, end_points_v3 = inception_v3.inception_v3(
            input_diversity(x_nes_2), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
        logits_v4_2, end_points_v4 = inception_v4.inception_v4(
            input_diversity(x_nes_2), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
        logits_res_v2_2, end_points_res_v2 = inception_resnet_v2.inception_resnet_v2(
            input_diversity(x_nes_2), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    with slim.arg_scope(resnet_v2.resnet_arg_scope()):
        logits_resnet_2, end_points_resnet = resnet_v2.resnet_v2_101(
            input_diversity(x_nes_2), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    logits_2 = (logits_v3_2 + logits_v4_2 + logits_res_v2_2 + logits_resnet_2) / 4
    cross_entropy_2 = tf.losses.softmax_cross_entropy(one_hot, logits_2, label_smoothing=0.0, weights=1.0)
    noise += tf.gradients(cross_entropy_2, x)[0]

    x_nes_4 = 1 / 4 * x
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v3_4, end_points_v3 = inception_v3.inception_v3(
            input_diversity(x_nes_4), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
        logits_v4_4, end_points_v4 = inception_v4.inception_v4(
            input_diversity(x_nes_4), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
        logits_res_v2_4, end_points_res_v2 = inception_resnet_v2.inception_resnet_v2(
            input_diversity(x_nes_4), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    with slim.arg_scope(resnet_v2.resnet_arg_scope()):
        logits_resnet_4, end_points_resnet = resnet_v2.resnet_v2_101(
            input_diversity(x_nes_4), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    logits_4 = (logits_v3_4 + logits_v4_4 + logits_res_v2_4 + logits_resnet_4) / 4
    cross_entropy_4 = tf.losses.softmax_cross_entropy(one_hot, logits_4, label_smoothing=0.0, weights=1.0)
    noise += tf.gradients(cross_entropy_4, x)[0]

    x_nes_8 = 1 / 8 * x
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v3_8, end_points_v3 = inception_v3.inception_v3(
           input_diversity(x_nes_8), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
        logits_v4_8, end_points_v4 = inception_v4.inception_v4(
            input_diversity(x_nes_8), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
        logits_res_v2_8, end_points_res_v2 = inception_resnet_v2.inception_resnet_v2(
            input_diversity(x_nes_8), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    with slim.arg_scope(resnet_v2.resnet_arg_scope()):
        logits_resnet_8, end_points_resnet = resnet_v2.resnet_v2_101(
            input_diversity(x_nes_8), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    logits_8 = (logits_v3_8 + logits_v4_8 + logits_res_v2_8 + logits_resnet_8) / 4
    cross_entropy_8 = tf.losses.softmax_cross_entropy(one_hot, logits_8, label_smoothing=0.0, weights=1.0)
    noise += tf.gradients(cross_entropy_8, x)[0]

    x_nes_16 = 1 / 16 * x
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v3_16, end_points_v3 = inception_v3.inception_v3(
            input_diversity(x_nes_16), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
        logits_v4_16, end_points_v4 = inception_v4.inception_v4(
            input_diversity(x_nes_16), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
        logits_res_v2_16, end_points_res_v2 = inception_resnet_v2.inception_resnet_v2(
            input_diversity(x_nes_16), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    with slim.arg_scope(resnet_v2.resnet_arg_scope()):
        logits_resnet_16, end_points_resnet = resnet_v2.resnet_v2_101(
            input_diversity(x_nes_16), num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    logits_16 = (logits_v3_16 + logits_v4_16 + logits_res_v2_16 + logits_resnet_16) / 4
    cross_entropy_16 = tf.losses.softmax_cross_entropy(one_hot, logits_16, label_smoothing=0.0, weights=1.0)
    noise += tf.gradients(cross_entropy_16, x)[0]

    noise = tf.nn.depthwise_conv2d(noise, stack_kernel, strides=[1, 1, 1, 1], padding='SAME')

    noise = noise / tf.reduce_mean(tf.abs(noise), [1, 2, 3], keep_dims=True)
    noise = tf.where(tf.is_nan(noise), tf.zeros_like(noise) , noise)


    mt = beta1 * mt + (1-beta1) * noise
    vt = beta2 * vt + (1-beta2) * tf.square(noise)

    
    normalized_grad = mt/(tf.sqrt(vt)+e)
    ...
    return x, y, i, x_max, x_min, noise, mt, vt


def stop(x, y, i, x_max, x_min, grad, mt, vt):
    num_iter = FLAGS.num_iter
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        i = tf.constant(0)
        mt = tf.zeros(shape=batch_shape)
        vt = tf.zeros(shape=batch_shape)
        grad = tf.zeros(shape=batch_shape)

        x_adv, _, _, _, _, _, _, _ = tf.while_loop(stop, graph, [x_input, y, i, x_max, x_min, grad, mt, vt])
