...
def graph(x, y, i, x_max, x_min, grad, grad2):
    ...
    # assert False, 1-tf.pow(beta2, tf.cast(i, dtype=tf.float32)+1)
    alpha_t = alpha_norm2 * num_iter * (tf.sqrt(1-tf.pow(beta2, tf.cast(i, dtype=tf.float32)+1)) / (1- tf.pow(beta1, tf.cast(i, dtype=tf.float32)+1))) / weight

    # assert False, alpha_t
