...
from third_party.feature_distillation import FD_jpeg_encode
from third_party.jpeg import jpeg_compress
...
if __name__ == '__main__':
    ...
    input_dir = './outputs_ai_si_ti_di_ens_fgm'
    ...
    with tf.Graph().as_default():
        # x_input_1 = tf.placeholder(tf.float32, shape=batch_shape)
        # x_input = bit_depth_reduce(x_input_1, -1, 1, 4, 200)
        x_input = tf.placeholder(tf.float32, shape=batch_shape)
        # x_input_1 = tf.placeholder(tf.float32, shape=batch_shape)
        # x_input = jpeg_compress(x_input_1, -1, 1)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            model_name = ['ens3_adv_inception_v3']
            ...
            for filenames, images in load_images(input_dir, batch_shape):
                ...
                ens3_adv_v3 = sess.run(pred_ens3_adv_v3, feed_dict={x_input: images})

                for filename, l1 in zip(filenames, ens3_adv_v3):
                    label = f2l[filename]
                    l = [l1]
